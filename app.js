...
let quoteNumberTimer;

function setLocalNextQuoteNumber() {
...
async function setNextQuoteNumber() {
  ...
  try {
    const response = await fetch(`/api/next-quote-number?${params.toString()}`);
    if (!response.ok) throw new Error('No se pudo obtener el correlativo');
    const result = await response.json();
    quoteNumber.value = result.quoteNumber || quoteNumber.value;
  } catch (error) {
    setLocalNextQuoteNumber();
  }
  updatePreview();
}

function scheduleNextQuoteNumber() {
  clearTimeout(quoteNumberTimer);
  quoteNumberTimer = setTimeout(setNextQuoteNumber, 350);
}
...
function advanceQuoteNumberForProfessional(nextQuoteNumberFromServer) {
  if (nextQuoteNumberFromServer) {
    quoteNumber.value = nextQuoteNumberFromServer;
    updatePreview();
    return;
  }
...
form.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    saveProfessionalProfile();
    const result = await response.json();
    advanceQuoteNumberForProfessional(result.nextQuoteNumber);
    statusEl.textContent = result.persistent
      ? 'Cotización guardada en PostgreSQL correctamente. Ya tienes listo el siguiente número correlativo.'
      : 'Cotización guardada temporalmente. Ya tienes listo el siguiente número correlativo.';
  } catch (error) {
